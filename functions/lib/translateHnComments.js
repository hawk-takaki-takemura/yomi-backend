...
async function collectCommentIdsBreadthFirst(rootIds, limit) {
    const queue = [...rootIds];
    ...
    while (queue.length > 0 && collected.length < limit) {
        ...
        if (!id || visited.has(id))
            continue;
        visited.add(id);
        const item = await (0, client_js_1.fetchItem)(id);
...
exports.translateHnComments = (0, https_1.onCall)({
    ...
    const commentItems = await Promise.all(commentIds.map((id) => (0, client_js_1.fetchItem)(id)));
    const commentTexts = {};
